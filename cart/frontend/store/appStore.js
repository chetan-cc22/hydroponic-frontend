...
export const useAppStore = create((set, get) => ({
  ...
  cart: null,
  totalCart: null,
  products: [], // Added products state to store fetched product data
  ...
  // **NEW ACTION: getAllProducts** for fetching product data
  getAllProducts: async () => {
    set({ loading: true });
    try {
      const { data } = await API.get("/api/products");  // <-- Make sure this matches your API endpoint
      set({ products: data, loading: false });  // Store fetched products in state
    } catch (e) {
      set({ loading: false });
